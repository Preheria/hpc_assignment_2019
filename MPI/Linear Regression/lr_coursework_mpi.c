...
int main() {
	...
	//initialize MPI
	MPI_Init (NULL, NULL);
	//finds no.of processors that will run the program
   	MPI_Comm_size (MPI_COMM_WORLD, &size);
	// gets process id(rank)
	...
	if (size != 9){
  	...
  	while(!minimum_found) {
		 if (rank != 0){
				...
				// sends slope, intercept and error obtained to root process i.e. 0
				...
				//receives optimal slope and intercept values for minmum error 
			...
			for(i=0;i<(size-1);i++) {
				// receives slopes and intercept from 1-8 processes
